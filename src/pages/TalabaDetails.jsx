...
const TalabaDetails = () => {
  ...
  useEffect(() => {
    const fetchTalaba = async () => {
      try {
        const response = await DataService.get(endpoints.TALABALARDETAILS(id));
        const guruhlar = await DataService.get(endpoints.GURUHLAR);
        response.guruh = guruhlar.find(
          (i) => i.id === response.guruhlarId,
        ).nomi;
        setTalaba(response);
        setError(null);
      } catch (err) {
        ...
        <div className="md:flex">
          <div className="md:flex-shrink-0">
            <img
              ...
              src={`../../public/img${talaba.imageUrl}`}
              alt={talaba.fio}
            />
          </div>
          <div className="p-8">
            ...
            <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="font-medium">{talaba.yoshi}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Jinsi:</p>
                <p className="font-medium">{talaba.jinsi}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Telefon:</p>
                <p className="font-medium">
                  <a
                    href={`tel:${talaba.telefon}`}
              ...
              <div>
                <p className="text-sm text-gray-500">Guruhi:</p>
                <p className="font-medium">{talaba.guruh}</p>
              </div>
            </div>

            <div className="mt-6">
              <p className="text-sm text-gray-500">Qo'shimcha ma'lumot:</p>
              <p className="mt-1 text-gray-600">{talaba.izoh}</p>
            </div>
